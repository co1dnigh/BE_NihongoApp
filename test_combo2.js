...
(async()=>{
 try{
  ...
  console.log('start status',s.status,'hasQuestions',Array.isArray(s.body.questions));
  if(!s.body.questions){console.log('start body',JSON.stringify(s.body).slice(0,300));return;}
  const qs=s.body.questions;
  const answers=qs.map((q,i)=>({questionId:q.questionId,isCorrect: i!==2}));
