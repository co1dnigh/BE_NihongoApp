...
inventoryItem.setExpiresAt(expiresAt);
...
@Transactional(readOnly = true)
public List<UserInventory> getActivePowerups(Long userId) {
return userInventoryRepository.findActivePowerupsByUserId(userId, LocalDateTime.now());
}

@Transactional(readOnly = true)
public boolean hasActivePowerup(Long userId, ShopItem.EffectType effectType) {
LocalDateTime now = LocalDateTime.now();
List<UserInventory> powerups = userInventoryRepository.findActivePowerupsByUserId(userId, now);

if (powerups.isEmpty()) {
return false;
}

Collection<Long> itemIds = powerups.stream()
.map(UserInventory::getItemId)
.distinct()
.toList();

List<ShopItem> shopItems = shopItemRepository.findByIdIn(itemIds);
Map<Long, ShopItem> itemMap = shopItems.stream()
.collect(Collectors.toMap(ShopItem::getId, si -> si));

return powerups.stream()
.anyMatch(ui -> {
ShopItem si = itemMap.get(ui.getItemId());
return si != null && si.getEffectType() == effectType;
});
}
